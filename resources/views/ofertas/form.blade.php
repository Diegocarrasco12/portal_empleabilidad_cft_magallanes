...
<section class="card">
    <form method="POST"
        ...
        enctype="multipart/form-data" class="form-create-oferta">
        @csrf
        @if ($editando)
            @method('PUT')
        @endif
            ...
            <div class="grid-2">
                    ...
                    <select id="area_id" name="area_id" required>
                        ...
                        @foreach ($areas as $area)
                            <option value="{{ $area->id }}"
                                {{ old('area_id', $editando ? $oferta->area_id : null) == $area->id ? 'selected' : '' }}>
                                {{ $area->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="field">
                    ...
                    <select id="tipo_contrato_id" name="tipo_contrato_id" required>
                        ...
                        <option value="1"
                            {{ old('tipo_contrato_id', $editando ? $oferta->tipo_contrato_id : '') == 1 ? 'selected' : '' }}>
                            Plazo fijo</option>
                        <option value="2"
                            {{ old('tipo_contrato_id', $editando ? $oferta->tipo_contrato_id : '') == 2 ? 'selected' : '' }}>
                            Indefinido</option>
                        <option value="3"
                            {{ old('tipo_contrato_id', $editando ? $oferta->tipo_contrato_id : '') == 3 ? 'selected' : '' }}>
                            Práctica</option>
                        <option value="4"
                            {{ old('tipo_contrato_id', $editando ? $oferta->tipo_contrato_id : '') == 4 ? 'selected' : '' }}>
                            Honorarios</option>
                    </select>
                ...
                <div class="field">
                    ...
                    <select id="modalidad_id" name="modalidad_id" required>
                        <option value="">Selecciona</option>
                        <option value="1"
                            {{ old('modalidad_id', $editando ? $oferta->modalidad_id : '') == 1 ? 'selected' : '' }}>
                            Presencial</option>
                        <option value="2"
                            {{ old('modalidad_id', $editando ? $oferta->modalidad_id : '') == 2 ? 'selected' : '' }}>
                            Remoto</option>
                        <option value="3"
                            {{ old('modalidad_id', $editando ? $oferta->modalidad_id : '') == 3 ? 'selected' : '' }}>
                            Híbrido</option>
                    </select>
                    @error('modalidad_id')
                        <small class="error">{{ $message }}</small>
            ...
            <div class="grid-3 mt-2">
                ...
                <div class="field">
                    ...
                    <select id="jornada_id" name="jornada_id">
                        <option value="">Selecciona</option>
                        <option value="1"
                            {{ old('jornada_id', $editando ? $oferta->jornada_id : '') == 1 ? 'selected' : '' }}>
                            Full-time</option>
                        <option value="2"
                            {{ old('jornada_id', $editando ? $oferta->jornada_id : '') == 2 ? 'selected' : '' }}>
                            Part-time</option>
                        <option value="3"
                            {{ old('jornada_id', $editando ? $oferta->jornada_id : '') == 3 ? 'selected' : '' }}>Por
                            turnos</option>
                    </select>
                </div>
                ...
                <div class="field">
                    ...
                    <input id="vacantes" name="vacantes" type="number" min="1"
                        value="{{ old('vacantes', $editando ? $oferta->vacantes : '') }}" placeholder="1">
                </div>
            ...
            <div class="grid-3">
                ...
                <div class="field">
                    ...
                    <input id="ciudad" name="ciudad" type="text"
                        value="{{ old('ciudad', $editando ? $oferta->ciudad : '') }}" placeholder="Punta Arenas">
                </div>
        ...
        <section class="card mt-3">
            ...
            <div class="grid-3">
                ...
                <div class="field">
                    ...
                    <input id="sueldo_min" name="sueldo_min" type="number" min="0" step="1000"
                        value="{{ old('sueldo_min', $editando ? $oferta->sueldo_min : '') }}" placeholder="700000">
                </div>

                <div class="field">
                    <label for="sueldo_max">Salario máximo (CLP)</label>
                    <input id="sueldo_max" type="number" min="0" step="1000" name="sueldo_max"
                        value="{{ old('sueldo_max', $editando ? $oferta->sueldo_max : '') }}" placeholder="1200000">
                </div>
                ...
                <div class="field">
                    ...
                    <select id="mostrar_sueldo" name="mostrar_sueldo">
                        <option value="1"
                            {{ old('mostrar_sueldo', $editando ? $oferta->mostrar_sueldo : '') == 1 ? 'selected' : '' }}>
                            Sí</option>
                        <option value="0"
                            {{ old('mostrar_sueldo', $editando ? $oferta->mostrar_sueldo : '') == 0 ? 'selected' : '' }}>
                            No</option>
                    </select>
                </div>
        ...
        <section class="card mt-3">
            ...
            <div class="field">
                ...
                <textarea id="descripcion" name="descripcion" rows="6" required
                    placeholder="Explica responsabilidades, equipo, tecnologías, herramientas, etc.">{{ old('descripcion', $editando ? $oferta->descripcion : '') }}</textarea>
            </div>
            ...
            <div class="grid-2 mt-2">
                ...
                <div class="field">
                    ...
                    <textarea id="requisitos" name="requisitos" rows="5" required
                        placeholder="Lista requisitos mínimos (bullets).">{{ old('requisitos', $editando ? $oferta->requisitos : '') }}</textarea>
                </div>

                {{-- Deseables --}}
                <div class="field">
                    <label for="habilidades_deseadas">Conocimientos deseables</label>
                    <textarea id="habilidades_deseadas" name="habilidades_deseadas" rows="5"
                        placeholder="Plus o deseables (bullets).">{{ old('habilidades_deseadas', $editando ? $oferta->habilidades_deseadas : '') }}</textarea>
                </div>
        ...
        <section class="card mt-3">
            ...
            <div class="grid-3">
                ...
                <div class="field">
                    ...
                    <input id="correo_contacto" name="correo_contacto" type="email" required
                ...
                <div class="field">
                    ...
                    <input id="telefono_contacto" name="telefono_contacto" type="tel"
